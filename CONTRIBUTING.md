...
## 1. Add a failure-pattern detector

A failure-pattern is a named failure-mode of LLM-driven multi-agent systems. The contribution shape is:

- a one-line phenotype (what does it look like in output?)
- a runnable detector function `(text: str) -> FailureModeHit` — see `agent_failure_modes/agent_failure_modes.py` for the signature
...
GitHub Discussions on this repo is the watering hole. Use it for: design questions, proposing a new failure-pattern before writing code, asking whether a check belongs in shared or in your own fork.

## Anti-sycophancy norm
